...
def makeFWTMPShuffle(dirName, n, stateNum,fdim):
    FrameNumList = [0]*n
...
def getFWTMP(dirName, stateNum, frameNum, fdim):
...
def makeModel(jsonName, h5Name, dirName, stateNum, frameNum, fdim=45):
    ...
    if  totalFrameSize>= 3000000000:
        multiStep = 1
        n = totalFrameSize/3000000000 + 1
...
def trainModel(jsonName, h5Name, dirName, stateNum, frameNum, learnRate, fdim):
    print "data loading!"
    ...
    print "train-------gModel"
    #model = getModel(jsonName, h5Name)
    sgd = SGD(lr=learnRate * 0.01, decay=1e-6, momentum=0.9, nesterov=True)
    print "json model:"
    ...
    if multiStep != 1:
        ...
        for iter in range(epoch):
            ...
            for i in range(0,n):
                data,label=getFWTMPmulti(dirName,stateNum, TotalFrameList[i],i,fdim)
                data = data.reshape(data.shape[0]/fdim, fdim)
                Model.fit(data,label,batch_size=100, nb_epoch=3,shuffle=True,verbose=1)   
                Model.save_weights(h5Name,overwrite=True)
                json_string = Model.to_json()
                open(jsonName,'w').write(json_string)
...
# print "data loading!"
# jsonName = 'model.jsonfull_8mix_0.cb'
# h5Name = 'model.h5full_8mix_0.cb'
# dirName ='FWTMP'
# stateNum = 3206
# frameNum = 32544675
# fdim = 495
...
# n = totalFrameSize/3000000000 + 1
# print "train-------gModel"
# sgd = SGD(lr=0.01 * 0.01, decay=1e-6, momentum=0.9, nesterov=True)
# time.sleep(10)
# print "json model:"
# Model = model_from_json(open(jsonName).read())
# print "json model end"
...
# data,label=getFWTMPmulti(dirName,stateNum, 1480979,0,fdim)
# data = data.reshape(data.shape[0]/fdim, fdim)
# Model.fit(data,label,batch_size=100, nb_epoch=1,shuffle=True,verbose=1)     
