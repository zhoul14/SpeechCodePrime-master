...
void randomunit(ae_int_t n, /* Real    */ ae_vector* x, ae_state *_state)
{
    ...
    do
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            m_ConMatrix = randomnormal(_state);
            x->ptr.p_double[i] = m_ConMatrix;
            v = v+m_ConMatrix*m_ConMatrix;
        }
    }
     ...
     ae_state *_state)
{
    ...
    *info = 0;
    ...
    ae_vector_init(&m_ConMatrix, 0, DT_REAL, _state, ae_true);
    ...
    ae_vector_set_length(&m_ConMatrix, ns+1+1, _state);
            ...
            for(j=i-ns+1; j<=i; j++)
            {
                if( ae_fp_greater_eq(wi->ptr.p_double[j],0) )
                {
                    if( ae_fp_eq(wi->ptr.p_double[j],0) )
                    {
                         ...
                         */
                        p1 = nv+1;
                        ae_v_move(&m_ConMatrix.ptr.p_double[1], 1, &v.ptr.p_double[1], 1, ae_v_len(1,p1));
                        matrixvectormultiply(h, l, l+nv, l, l+nv-1, ae_false, &m_ConMatrix, 1, nv, 1.0, &v, 1, nv+1, -wr->ptr.p_double[j], _state);
                        nv = nv+1;
                    }
                    else
                    {
                        if( ae_fp_greater(wi->ptr.p_double[j],0) )
                        {
                            ...
                            p1 = nv+1;
                            ae_v_move(&m_ConMatrix.ptr.p_double[1], 1, &v.ptr.p_double[1], 1, ae_v_len(1,p1));
                            matrixvectormultiply(h, l, l+nv, l, l+nv-1, ae_false, &v, 1, nv, 1.0, &m_ConMatrix, 1, nv+1, -2*wr->ptr.p_double[j], _state);
                            itemp = vectoridxabsmax(&m_ConMatrix, 1, nv+1, _state);
                            temp = 1/ae_maxreal(ae_fabs(m_ConMatrix.ptr.p_double[itemp], _state), smlnum, _state);
                            p1 = nv+1;
                            ae_v_muld(&m_ConMatrix.ptr.p_double[1], 1, ae_v_len(1,p1), temp);
                            absw = pythag2(wr->ptr.p_double[j], wi->ptr.p_double[j], _state);
                            temp = temp*absw*absw;
                            matrixvectormultiply(h, l, l+nv+1, l, l+nv, ae_false, &m_ConMatrix, 1, nv+1, 1.0, &v, 1, nv+2, temp, _state);
                            nv = nv+2;
                        }
                    }
