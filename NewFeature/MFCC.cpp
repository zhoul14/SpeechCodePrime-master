...
float CMFCC::DoMFCC(float *inData, float *outVect)
{
int		i, SubBandNo;
...
float CMFCC::DoCepMFCC(short *inData, float *outVect)
{
int		i, SubBandNo;
float	FrameEnergy,ek,temp1;
float	*pCosTable;
...
double	ener;
	...
	for( i = FrameLen; i< FFT_LEN; i++ ) 
	{
		FFTFrame[i] = 0.0f;
		ImgBuf[i]	= 0;
	}
	// 进行FFT变换
	DoFFT(FFTFrame,ImgBuf,0);					//对输入信号进行FFT变换
	for( i = 0; i < FFT_LEN; i++ )				//计算信号的幅度谱
	{
		ener = FFTFrame[i]*FFTFrame[i]+ImgBuf[i]*ImgBuf[i];
		FFTFrame[i] = (float)sqrt(ener);
	}

	for( i = 0; i < FFT_LEN; i++ )				//计算信号的幅度对数谱
	{
		if( FFTFrame[i] <= 0 ) FFTFrame[i]=0.0001f;
		FFTFrame[i] = (float)log(FFTFrame[i]);
		ImgBuf[i] = 0;
	}
	DoFFT( FFTFrame , ImgBuf , 1 );				//用IFFT计算倒谱系数

	for( i=32; i<FFT_LEN-32; i++ )				//取倒谱的低时域分量
	{
		FFTFrame[i]=0;
	}
	for(i=0;i<FFT_LEN;i++)
	...
	DoFFT( FFTFrame, ImgBuf, 0 );				//计算声道的对数谱
	
	for( i=0; i<FFT_LEN; i++ )					//恢复声道的幅度谱
	{
		FFTFrame[i]=(float)exp( FFTFrame[i] );
	}
...
float CMFCC::DoSMFCC(short *inData, float *outVect)
{
int		i, SubBandNo;
float	FrameEnergy,ek,temp1,temp2;
float	*pCosTable;
...
float CMFCC::DoSSMFCC(short *inData, float *outVect)
{
int		i, SubBandNo;
float	FrameEnergy,ek,temp1,temp2;
float	*pCosTable;
